					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							...
							if (item.text != "undefined") {
								...
								else mHTML="<img src='"+item.profile_image_url+"' alt='"+item.from_user+"' class='avatar' /><a href='http://www.twitter.com/"+item.from_user+"'>"+item.from_user+"<br /></a> "+$.Juitter.textFormat(tweet)+" <div class='time'>"+item.created_at+"</div><div class='clear'></div>";
