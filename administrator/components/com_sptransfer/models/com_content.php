...
class SPTransferModelCom_Content extends JModel
{
    protected $jAp;   
    protected $tableLog;
    ...
    public function categories($ids = null)    {
        ...
        $message= ('<h2>'.JText::_(COM_CONTENT).' - '.JText::_(COM_CONTENT_CATEGORIES).'</h2>');
        $this->writeLog($message); 
    ...
    public function content($ids = null)    {
        ...
        $message= ('<h2>'.JText::_(COM_CONTENT).' - '.JText::_(COM_CONTENT_CONTENT).'</h2>');
        $this->writeLog($message); 
    ...
    public function content_fix($ids = null)    {
        ...
        foreach ($items as $i => $item) {
            ...
            if ($item['catid'] > 1) {
                $tableLog->reset();
                $tableLog->id = null;
                $tableLog->load(array("tables_id" => 4, "source_id" => $item['catid']));
                if ($tableLog->source_id == $tableLog->destination_id) {
                    $tableLog->load(array("tables_id" => $task->id, "destination_id" => $item['id']));            
                    $tableLog->state = 4;
                    $tableLog->store();
