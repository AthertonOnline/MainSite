...
class SPTransferModelCom_Newsfeeds extends JModel
{
    protected $jAp;   
    protected $tableLog;
    ...
    public function categories($ids = null)    {
        ...
        $message= ('<h2>'.JText::_(COM_NEWSFEEDS).' - '.JText::_(COM_NEWSFEEDS_CATEGORIES).'</h2>');
        $this->writeLog($message); 
        ...
        $query = 'SELECT * 
            FROM #__categories
            WHERE extension LIKE "com_newsfeeds"';        
    ...
    public function newsfeeds($ids = null)    {
        ...
        $destination_table = $this->getTable('Newsfeed', 'NewsfeedsTable');
        $user = $this->user;
        $params = $this->params;
        $task = $this->task;
        
        $message= ('<h2>'.JText::_(COM_NEWSFEEDS).' - '.JText::_(COM_NEWSFEEDS_NEWSFEEDS).'</h2>');
        $this->writeLog($message); 
    ...
    public function newsfeeds_fix($ids = null)    {
        ...
        foreach ($items as $i => $item) {
            ...
            if ($item['catid'] > 1) {
                $tableLog->reset();
                $tableLog->id = null;
                $tableLog->load(array("tables_id" => 10, "source_id" => $item['catid']));
                if ($tableLog->source_id == $tableLog->destination_id) {
                    $tableLog->load(array("tables_id" => $task->id, "destination_id" => $item['id']));            
                    $tableLog->state = 4;
                    $tableLog->store();
