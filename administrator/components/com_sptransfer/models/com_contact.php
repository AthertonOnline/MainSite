...
class SPTransferModelCom_Contact extends JModel
{
    protected $jAp;   
    protected $tableLog;
    ...
    public function categories($ids = null)    {
        ...
        $message= ('<h2>'.JText::_(COM_CONTACT).' - '.JText::_(COM_CONTACT_CATEGORIES).'</h2>');
        $this->writeLog($message); 
        ...
        $query = 'SELECT * 
            FROM #__categories
            WHERE extension LIKE "com_contact"';        
    ...
    public function contact_details($ids = null)    {
        ...
        $destination_table = $this->getTable('Contact', 'ContactTable');
        $user = $this->user;
        $params = $this->params;
        $task = $this->task;
        
        $message= ('<h2>'.JText::_(COM_CONTACT).' - '.JText::_(COM_CONTACT_CONTACT_DETAILS).'</h2>');
        $this->writeLog($message); 
    ...
    public function contact_details_fix($ids = null)    {
        ...
        foreach ($items as $i => $item) {
            ...
            //created_by
            if ($item['user_id'] > 0) {
                $tableLog->load(array("tables_id" => 3, "source_id" => $item['user_id']));
                if ($tableLog->source_id != $tableLog->destination_id) {
            ...
            if ($item['catid'] > 1) {
                $tableLog->reset();
                $tableLog->id = null;
                $tableLog->load(array("tables_id" => 6, "source_id" => $item['catid']));
                if ($tableLog->source_id == $tableLog->destination_id) {
                    $tableLog->load(array("tables_id" => $task->id, "destination_id" => $item['id']));            
                    $tableLog->state = 4;
                    $tableLog->store();
